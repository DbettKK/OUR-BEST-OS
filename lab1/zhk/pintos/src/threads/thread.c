...
/* 
  线程优先级调度即需要在就绪队列中维护一个优先队列
  根据线程的priority字段来确定顺序，并在线程进入就绪队列时就进行维护
  线程进入队列的时机:
    * 线程初始化时——init_thread()
    * 线程yeild时——thread_yeild()
    * 线程从阻塞状态被唤醒时——thread_unblock()
  可观察到这些函数都是调用的list_push_back()方法，该函数定义在lib/kernel目录中的list.c和list.h文件
  这里面还定义了list的结构体以及对应的其他list_insert方法 可以看到list_insert_ordered()方法
  直接使用该方法即可 只需要我们提供一个比较函数 参考qsort的cmp函数
  如下 定义线程优先级比较函数
  * 需要注意的是 list_entry宏的使用
*/
...
thread_set_priority (int new_priority) 
{
  ...
  int old_pri = cur_t->priority;
  cur_t->original_pri = new_priority;
  if (list_empty(&cur_t->lock_list) || new_priority > old_pri) {
...
bool is_idle(struct thread* cur_t) {
  return cur_t == idle_thread;
}
...
/*
1.  在一个线程获取一个锁的时候，如果拥有这个锁的线程优先级比自己低就提高它的优先级，并且如果这个锁还被别的锁锁着， 
将会递归地捐赠优先级， 然后在这个线程释放掉这个锁之后恢复未捐赠逻辑下的优先级。
2. 如果一个线程被多个线程捐赠， 维持当前优先级为捐赠优先级中的最大值（acquire和release之时）。
3. 在对一个线程进行优先级设置的时候， 如果这个线程处于被捐赠状态， 则对original_priority进行设置， 
然后如果设置的优先级大于当前优先级， 则改变当前优先级， 否则在捐赠状态取消的时候恢复original_priority。
4. 在释放锁对一个锁优先级有改变的时候应考虑其余被捐赠优先级和当前优先级。
5. 将信号量的等待队列实现为优先级队列。
6. 将condition的等待队列实现为优先级队列。
